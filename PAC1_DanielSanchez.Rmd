...
El primer component explica un 90.2 % de la variabilitat, mentre que el segon un 4.9 %.
...
```{r}
PCAS_met3$rotation[,1]
```

### Interpretació dels components

fer sobre l'1 $PC3 = 0.027922349 * X1.6.Anhydro.beta.D.glucose + 0.028158536 * X1.Methylnicotinamide + 0.033941035 * X2.Aminobutyrate + 0.033118667 * X2.Hydroxyisobutyrate + 0.013516382 * X2.Oxoglutarate + ...$

```{r}
PCAS_met2$loadings[,1][names(sort(abs(PCAS_met2$loadings[,1]), decreasing = TRUE))]
```

Explicar el % dels components

les variables d'aquest coimponent les pes mes grans son.. 
Tres metodes semblant, interpeto el del codi
...
Repetir l'analisi dels components
